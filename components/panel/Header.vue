<template>
    <header class="sticky top-0  z-999999 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
      <div class="flex flex-grow items-center justify-between lg:justify-end py-4 px-4 shadow-2 md:px-6 2xl:px-11">
        <div class="flex items-center gap-2 sm:gap-4 lg:hidden">
          ...
          <button
            ...
            @click.stop="$emit('showSidebarHandler', !props.sidebarToggle)">
        ...
        <div class="flex items-center gap-3 2xsm:gap-7 m-l">
          <ul class="flex items-center gap-2 2xsm:gap-4">
            <li>
              <!-- Dark Mode Toggler -->
              <label :class="darkMode ? 'bg-primary' : 'bg-stroke'" class="relative m-0 block h-7.5 w-14 rounded-full">
                <input type="checkbox" :value="darkMode" @change="darkMode = !darkMode"
                  @change.prevent="colorMode.value === 'dark' ? colorMode.preference = 'light' : colorMode.preference = 'dark'"
                  class="absolute top-0 z-50 m-0 h-full w-full cursor-pointer opacity-0" />
                <span :class="darkMode && '!left-[3px] !translate-x-full'"
